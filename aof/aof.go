...
// bufioReader with tail
type bufioTailReader struct {
	input        *bufio.Reader
	spinInterval uint32
}
...
func NewBufioTailReader(reader io.Reader, intval uint32) Reader {
	input := bufio.NewReader(reader)
	return bufioTailReader{input: input, spinInterval: intval}
}
...
func (reader bufioTailReader) readLine() (s string, err error) {
retry:
	str, err := reader.input.ReadString('\n')
	if err == io.EOF || len(str) == 0 {
		time.Sleep(time.Duration(reader.spinInterval) * time.Millisecond)
		goto retry
	}
	if err != nil {
		return
...
func (reader bufioTailReader) readParameter() (s string, err error) {
	// read parameter length
	str, err := reader.readLine()
	if err != nil {
		return
...
func (reader bufioTailReader) ReadOperation() (op Operation, err error) {
	// read parameter count
	var key string
	var subop string
	str, err := reader.readLine()
